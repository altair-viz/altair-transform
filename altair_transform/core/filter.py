...
@eval_predicate.register
def eval_field_equal(predicate: alt.FieldEqualPredicate,
                     df: pd.DataFrame) -> pd.Series:
    return get_column(df, predicate) == eval_value(predicate.equal)


@eval_predicate.register
def eval_field_range(predicate: alt.FieldRangePredicate,
                     df: pd.DataFrame) -> pd.Series:
...
@eval_predicate.register
def eval_field_oneof(predicate: alt.FieldOneOfPredicate,
                     df: pd.DataFrame) -> pd.Series:
...
@eval_predicate.register
def eval_field_lt(predicate: alt.FieldLTPredicate,
                  df: pd.DataFrame) -> pd.Series:
    return get_column(df, predicate) < eval_value(predicate.lt)


@eval_predicate.register
def eval_field_lte(predicate: alt.FieldLTEPredicate,
                   df: pd.DataFrame) -> pd.Series:
    return get_column(df, predicate) <= eval_value(predicate.lte)


@eval_predicate.register
def eval_field_gt(predicate: alt.FieldGTPredicate,
                  df: pd.DataFrame) -> pd.Series:
    return get_column(df, predicate) > eval_value(predicate.gt)


@eval_predicate.register
def eval_field_gte(predicate: alt.FieldGTEPredicate,
                   df: pd.DataFrame) -> pd.Series:
    return get_column(df, predicate) >= eval_value(predicate.gte)


@eval_predicate.register
def eval_logical_not(predicate: alt.LogicalNotPredicate,
                     df: pd.DataFrame) -> pd.Series:
    return ~eval_predicate(predicate['not'], df)


@eval_predicate.register
def eval_logical_and(predicate: alt.LogicalAndPredicate,
                     df: pd.DataFrame) -> pd.Series:
...
@eval_predicate.register
def eval_logical_or(predicate: alt.LogicalOrPredicate,
                    df: pd.DataFrame) -> pd.Series:
